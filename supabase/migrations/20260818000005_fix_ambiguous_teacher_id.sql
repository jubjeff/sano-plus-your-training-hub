-- Corrige "column reference "teacher_id" is ambiguous" em provision_current_teacher_account.
--
-- A funcao declara `returns table (teacher_id uuid, ...)`. Em PL/pgSQL cada coluna
-- de retorno vira uma variavel no escopo do corpo, entao o `teacher_id` solto em
--
--   select 1 from public.teacher_subscriptions where teacher_id = teacher_record.id
--
-- ficava ambiguo entre a variavel de saida e a coluna teacher_subscriptions.teacher_id,
-- e o Postgres aborta com 42702. O erro aparecia ao salvar o perfil do professor:
-- authService chama esta RPC no fluxo de provisionamento (auth.service.ts).
--
-- Correcao: alias explicito na subquery (`teacher_subscriptions ts` / `ts.teacher_id`).
-- Unica ocorrencia do padrao no projeto — varri todas as funcoes com OUT params.
-- O restante do corpo e identico ao de 20260418010500.
...
begin
  ...
  if not exists (select 1 from public.teacher_subscriptions ts where ts.teacher_id = teacher_record.id) then
